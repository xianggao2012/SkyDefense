...
void GameLayer::resetMeteor() {
    ...
    //  meteor->setPosition(Point{static_cast<float>(meteor_x), _screenSize.height + meteor->boundingBox().size.height * 0.5});
    meteor->setPosition(Point{static_cast<float>(meteor_x), static_cast<float>(_screenSize.height + meteor->boundingBox().size.height * 0.5)});
    
    auto rotate = RotateBy::create(0.5f, -90);
    auto rotateForever = RepeatForever::create(rotate);
    auto sequence = Sequence::create(MoveTo::create(_meteorSpeed, Point{static_cast<float>(meteor_target_x), _screenSize.height * 0.15f}),
                                     CallFuncN::create(
...
void GameLayer::resetHealth() {
    ...
    health->setPosition(Point{static_cast<float>(health_x), static_cast<float>(_screenSize.height + health->boundingBox().size.height * 0.5)});
    
    auto sequence = Sequence::create(
                                     MoveTo::create(_healthSpeed, Point{static_cast<float>(health_target_x), _screenSize.height * 0.15f}),
                                     CallFuncN::create(
                                                       std::bind(&GameLayer::fallingObjectDone, this, std::placeholders::_1)),
                                     nullptr
                                     );
